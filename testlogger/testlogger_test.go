...
func TestNew(t *testing.T) {
	t.Run("ReturnsLoggerAndWriter", func(t *testing.T) {
		logger, writer := testlogger.New()
		
		assert.NotNil(t, logger)
		assert.NotNil(t, writer)
		assert.IsType(t, zerolog.Logger{}, logger)
		assert.IsType(t, &testlogger.BufWriter{}, writer)
	})

	t.Run("LoggerAndWriterAreProperlyConnected", func(t *testing.T) {
		logger, writer := testlogger.New()
		
		// The logger and writer should be connected, but zerolog writes JSON directly
		// which doesn't match the expected wrapped format of BufWriter
		// This test just verifies they're properly initialized and connected
		assert.NotNil(t, logger)
		assert.NotNil(t, writer)
